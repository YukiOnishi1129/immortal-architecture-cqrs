...
// NoteCommandRepository implements command (write) persistence for notes.
type NoteCommandRepository struct {
	pool    *pgxpool.Pool
	queries *generated.Queries
}

var _ port.NoteCommandRepository = (*NoteCommandRepository)(nil)

// NewNoteCommandRepository creates NoteCommandRepository.
func NewNoteCommandRepository(pool *pgxpool.Pool) *NoteCommandRepository {
	return &NoteCommandRepository{
		pool:    pool,
		queries: generated.New(pool),
	}
}

// Get returns a note with sections (used for validation in command operations).
func (r *NoteCommandRepository) Get(ctx context.Context, id string) (*note.WithMeta, error) {
	pgID, err := toUUID(id)
	if err != nil {
		return nil, err
	}
...
func (r *NoteCommandRepository) Create(ctx context.Context, n note.Note) (*note.Note, error) {
...
func (r *NoteCommandRepository) Update(ctx context.Context, n note.Note) (*note.Note, error) {
...
func (r *NoteCommandRepository) UpdateStatus(ctx context.Context, id string, status note.NoteStatus) (*note.Note, error) {
	pgID, err := toUUID(id)
	if err != nil {
		return nil, err
	}
...
func (r *NoteCommandRepository) Delete(ctx context.Context, id string) error {
	pgID, err := toUUID(id)
	if err != nil {
		return err
	}
...
func (r *NoteCommandRepository) ReplaceSections(ctx context.Context, noteID string, sections []note.Section) error {
...
func (r *NoteCommandRepository) listSections(ctx context.Context, noteID pgtype.UUID) ([]note.SectionWithField, error) {
